...
pub fn generate(out_dir: &str) -> Result<(), String> {
  ...
  let man_path = gen_dir.join("flint.1");
  let mut buffer: Vec<u8> = Vec::new();

  let mut cmd = flint::Cli::command();
  let mut man = clap_mangen::Man::new(cmd.clone());
  man = man.manual("flint manual".to_string());
  man
  ...
  let env_vars = [
    (
      "FLINT_FLAKE_PATH",
      "Default path to the directory containing flake.nix for all commands.",
    ),
    (
      "FLINT_CMD_TIMEOUT",
      "Command timeout in milliseconds for Nix and git subprocesses.",
    ),
    (
      "FLINT_UPDATE_THRESHOLD",
      "Stale input threshold in seconds for the stale subcommand.",
    ),
    (
      "FLINT_LOG_LEVEL",
      "Tracing filter level (e.g., flint=trace). Overrides verbosity flags.",
    ),
  ];
  let mut sect = Roff::new();
  ...
  let files = [
    (
      "flake.nix",
      "Primary flake definition in the target directory.",
    ),
    (
      "flake.lock",
    ...
    (
      "flake.nix.bak",
      "Backup created when running duplicates --fix with backups enabled.",
    ),
  ];
  let mut sect = Roff::new();
  ...
  // EXAMPLES section
  let examples = [
    ("Check inputs for staleness", "flint stale"),
    (
    ...
    (
      "Set the flake path with an env var",
