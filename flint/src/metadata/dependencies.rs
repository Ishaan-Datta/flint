...
pub fn get_input_deps(
    ...
    if output.status.success() {
        ...
        if deps_map.is_empty() {
            tracing::warn!(
                "Found empty input dependencies map, is the flake blank?"
            );
        ...
        let root_entry = if let Some(val) = deps_map.get("root") {
            ...
            tracing::warn!(
                "Missing flake root entry, can't validate user_inputs"
            );
        ...
        for (input, deps) in deps_map {
            ...
            for (dependency, target) in deps {
                if target.len() > 1 || target.is_empty() {
                    tracing::trace!(
                        "[{input}]: Found irregular target: {target:?} for \
                         {dependency}"
                    );
                    continue;
                }
                ...
                if ends_with_2_to_99(&target[0]) {
                    let new_target = format!(
                        "inputs.{dependency}.follows = \"{dependency}\";"
                    );
                    tracing::trace!(
                        "[{input}]: Setting target for {dependency}: \
                         {new_target}"
                    );
                    let replacement = InputReplacement::new(
                        &dependency,
                        &target[0],
                        &new_target,
                    );
