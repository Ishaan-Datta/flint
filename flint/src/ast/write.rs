...
pub fn rewrite_flake_inputs(
  ...
  if fix {
    ...
    let new_flake_content = apply_flake_input_edits(&flake_content, &converted)
      .unwrap_or_else(|e| {
...
pub(crate) fn write_new_flake_file(
  ...
  let output =
    with_command_spinner!("Validating the flake file edits", &cmd, timeout)?;
  ...
  for (input_name, lines) in input_dep_edits {
      ...
      find_attrset_binding_by_name(inputs_rhs, flake_file_content, input_name)
    {
      ...
      if already_nested {
        ...
        let edit =
          insert_into_existing_attrset(rhs, flake_file_content, &missing)
